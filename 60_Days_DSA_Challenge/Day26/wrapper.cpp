...
After query 4, sum from index 1 to index
               ...
               Method 1 : (Brute Force)Implement three function, rotateR(arr, k) which will right rotate array arr by k times, rotateL(arr, k) which will left rotate array arr by k times, sum(arr, l, r) which will output sum of array arr from index l to index r. On the input of value 1, 2, 3 call the appropriate function.
...
We just need to track the net rotation. If the tracked number is negative, it means left rotation has dominated else right rotation has dominated. When we are tracking the net rotations, we need to do mod n. As after every n rotation, array will return to its original state.
